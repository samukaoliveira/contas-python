...
def carregar_fatura_com_rotativo(cartao: Cartao, competencia):
    ...
    # 🔥 regra nova: só gera rotativo para o próximo mês real
    hoje = datetime.date.today()

    prox = competencia_service.proximo(hoje.month, hoje.year)
    comp_proxima_real = (prox['mes'], prox['ano'])

    comp_fatura = (competencia.mes, competencia.ano)

    if comp_fatura == comp_proxima_real:
...
def get_proxima_fatura(fatura: Fatura):
    proxima_comp = competencia_service.proximo(
        fatura.competencia.mes,
        fatura.competencia.ano
    )

    comp_proxima = competencia_service.obter_ou_criar_competencia(
        proxima_comp['mes'],
        proxima_comp['ano']
    )
...
def gerar_lancamento_rotativo(fatura_anterior, fatura_atual, juros_mensal=0):
    """
    Gera rotativo baseado no saldo da fatura anterior.
    Evita cascata infinita.
    """

    # 🔁 garante consistência da fatura anterior
    if not fatura_anterior.lancamento_set.filter(eh_rotativo=True).exists():
        ...
        fatura_2_atras = Fatura.objects.filter(
            cartao_id=fatura_anterior.cartao_id,
    ...
    # ✅ gera rotativo da anterior → atual
...
def _gerar_rotativo_simples(fatura_anterior, fatura_atual, juros_mensal=0):
    """
    Calcula e aplica (ou remove) o rotativo.
    """

    totais = fatura_anterior.lancamento_set.aggregate(
        total_despesas=Coalesce(
            Sum('valor', filter=Q(natureza=Lancamento.Natureza.DESPESA)),
            Decimal('0')
        ),
        total_receitas=Coalesce(
            Sum('valor', filter=Q(natureza=Lancamento.Natureza.RECEITA)),
            Decimal('0')
        ),
    )
    ...
    # ❌ não tem dívida → remove rotativo
    if saldo <= 0:
